...
const Profile = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/accounts/create_profile/`, {
        method: "POST",
      ...
      if (response.ok) {
        ...
        setErrorMessage(data.detail || "Server error while creating profile");
      }
    } catch (error) {
      setErrorMessage("An error occurred while trying to create profile");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={profileStyles.profileContainer}>
      <h3 className="font-bold mt-4 text-xl text-red-500">Set up your profile</h3>
